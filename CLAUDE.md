# Claude Code Project Instructions — lru-kotlin

## Project Overview

This is **lru-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`lru`](https://crates.io/crates/lru).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/lru/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/lru>
